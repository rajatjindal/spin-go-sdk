...
func (m *Builder) TestIncomingHttp(ctx context.Context, source *dagger.Directory) error {
	svc := m.BuildExample(ctx, source, "http").
		WithWorkdir(fmt.Sprintf("/app/v2/examples/%s", "http")).
...
func (m *Builder) Spin(version string) *dagger.Container {
	...
	return dag.Container().
		...
		WithFile(
			"/usr/local/bin/spin",
			dag.Container().
				From("ghcr.io/fermyon/spin:canary-distroless").
				WithoutEntrypoint().
				File("/usr/local/bin/spin"),
		).Terminal()
}
